...
public static class PharmacyAggregatorTool
{
    [McpServerTool(Title = "Search for the best prices for drugs and their prices in multiple pharmacies"), Description("""
                                ...
                                """)]
    public static async Task<IEnumerable<PharmacyDrugInfo>> FindDrugPricesAsync([Description("The drug name.")] string drugName, McpServer mcpServer)
    {
        var logger = mcpServer.AsClientLoggerProvider().CreateLogger("PharmacyLogger");

        PharmacyExtraInfo? extraInfo = await TryLoadPharmacyExtraInfoAsync(mcpServer, logger);
    ...
    private static async Task<PharmacyExtraInfo?> TryLoadPharmacyExtraInfoAsync(McpServer mcpServer, ILogger logger)
    {
        try
        {
            //var elicitationResult = await mcpServer.ElicitAsync<PharmacyExtraInfo>("Please provide which pharmacies should I have to check.");
            ...
            if (elicitResult.Content == null) return null;
